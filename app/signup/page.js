...
const MediaHubRegistration = () => {
  // Note: Changed default to reader (false) to show News Reader first
  ...
  const [formData, setFormData] = useState({
    ...
    profilePic: null,
    agreeToTerms: false,
    role: 'reader', // Changed default to reader
    
    // Publisher specific fields (only added when switching to publisher)
    companyName: '',
    industry: '',
    companyWebsite: '',
    contactName: '',
  ...
  useEffect(() => {
    const handleGoogleCallback = (response) => {
      handleGoogleSignUp(response, formData, profilePicPreview, router, setIsLoading);
    };
  ...
  const handleGoogleSignUpButtonClick = () => {
    handleGoogleSignUpClick(formData);
  };
