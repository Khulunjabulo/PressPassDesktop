...
import { 
  ...
  Trash2, ExternalLink, Calendar, Globe, Users, X
} from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[85vh] flex flex-col shadow-2xl">
          ...
          {!loading && !error && filtered.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-sm">
                {search ? `No publishers matching "${search}"` : 'No publishers found'}
              </p>
              {search && (
                <button
                  onClick={() => setSearch('')}
          ...
          {!loading && !error && filtered.length > 0 && (
            <ul className="divide-y divide-gray-100">
              {filtered.map((publisher) => (
                <li
                  key={publisher.id}
                  className="flex items-center space-x-4 py-3.5 hover:bg-gray-50 rounded-xl px-2 -mx-2 transition-colors"
                >
                  {/* Logo / Initial */}
                  <div className="flex-shrink-0 w-11 h-11 rounded-xl overflow-hidden border border-gray-200 bg-gradient-to-br from-gray-100 to-gray-200">
                    {publisher.logo ? (
                      <img
                        src={publisher.logo}
                        alt={publisher.name}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-2 mt-0.5">
                      <span className="text-xs text-gray-500 capitalize truncate">
                        {publisher.publicationType || 'News'}
                      </span>
                      {publisher.industry && (
                        <>
        ...
        <div className="px-5 py-3 border-t border-gray-100 flex-shrink-0 flex items-center justify-between">
          <p className="text-xs text-gray-400">
            Click the heart to add publishers to your favorites
          </p>
          <button
            onClick={onClose}
