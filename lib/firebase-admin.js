  ...
  if (!app) {
    try {
      ...
      app = admin.initializeApp({
        ...
        projectId: process.env.FIREBASE_PROJECT_ID,
        storageBucket: process.env.FIREBASE_STORAGE_BUCKET, // Keep this for backward compatibility
      });
...
// ⚠️ DEPRECATED: Keeping for backward compatibility
// This will be removed once all endpoints are migrated to Cloudinary
export function getStorage() {
  try {
    const firebaseApp = getFirebaseApp();
    const storage = admin.storage(firebaseApp);
    console.log('⚠️ [DEPRECATED] getStorage() called - consider migrating to Cloudinary');
    return storage;
  } catch (error) {
