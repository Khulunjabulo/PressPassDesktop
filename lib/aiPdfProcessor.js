...
class AIPdfProcessor {
  ...
  async processPDF(file, options = {}) {
    try {
      ...
      ('📄 Extracted text length:', fullText.length);
      ...
      ('📰 Detected stories:', stories.length);
      
      const storiesWithImages = await this.matchImagesToStories(stories, images, fullText);
      
      ('✅ PDF processing complete:', storiesWithImages.length, 'stories detected');
  ...
  async extractImagesFromPDF(file) {
    try {
      ...
      ('📷 Extracted', images.length, 'images from PDF');
      return images;
  ...
  smartFallbackDetection(fullText) {
    ('🧠 Using smart pattern detection (no AI needed)');
    ...
    ('🔍 Found', articleBoundaries.length, 'potential article headlines');
    ...
    if (articleBoundaries.length > 1) {
      ...
      ('✅ Smart detection found:', stories.length, 'articles');
      stories.forEach((s, i) => (`  ${i + 1}. ${s.headline}`));
      
      return stories;
    }
    
    // Fallback: Single article
    ('📄 Treating as single article');
    return [{
