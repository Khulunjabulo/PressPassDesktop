...
class AIPdfProcessor {
  ...
  async processPDF(file, options = {}) {
    try {
      ...
      // ✅ Clean up content paragraphs for every story
      stories = stories.map(story => ({
        ...story,
        content: this.cleanContentParagraphs(story.content),
      }));
  ...
  // ─── Clean content: join fragmented lines into proper paragraphs ─────────────
  // PDF text extraction often produces one word or half-sentence per line.
  // This joins those fragments into flowing paragraphs.
  cleanContentParagraphs(rawContent) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // 1. Line ends with sentence-ending punctuation AND next line starts a new sentence
      // 2. There's a blank line (already filtered, so we detect topic changes)
      // 3. Next line looks like a new heading (ALL CAPS short line)
      const endsWithPunctuation = /[.!?:]\s*$/.test(line);
      const nextIsHeading = nextLine && nextLine === nextLine.toUpperCase() && nextLine.length < 80 && nextLine.length > 5;
      const nextStartsNewSentence = nextLine && /^[A-Z]/.test(nextLine) && endsWithPunctuation;
      const isLastLine = i === lines.length - 1;

      if (isLastLine || nextIsHeading || (endsWithPunctuation && nextStartsNewSentence)) {
    ...
    // Return as HTML paragraphs so the template renders them with proper spacing
    return paragraphs
...
11. CRITICAL: The content field must be full flowing paragraphs of the article body — NOT line-by-line fragments. Join sentences and lines that belong together into complete paragraphs.
...
[
  {
    ...
    "category": "news",
    "content": "Full article body as flowing paragraphs. Join lines that belong to the same sentence. Do not leave single words or half-sentences on separate lines."
  }
]
...
${textToAnalyze}`;
    ...
    try {
      ...
      if (arrayMatch) stories = JSON.parse(arrayMatch[0]);
      else throw new Error('Could not parse AI response as JSON');
    }

    if (!Array.isArray(stories)) throw new Error('AI response is not an array');
  ...
  extractByline(lines) {
    for (const line of lines) {
      ...
      if (/^by\s+.+/i.test(trimmed)) return trimmed.replace(/^by\s+/i, '').trim();
      if (/^[A-Z][A-Z\s\-]+$/.test(trimmed) && trimmed.length > 4 && trimmed.length < 80 && trimmed.split(' ').length >= 2) {
        return this.toTitleCase(trimmed);
      }
      if (/^written by\s+.+/i.test(trimmed)) return trimmed.replace(/^written by\s+/i, '').trim();
    }
    return '';
  }
  ...
  guessCategory(text) {
    ...
    let bestCategory = 'news', bestScore = 0;
    for (const [category, keywords] of Object.entries(categories)) {
      const score = keywords.reduce((acc, kw) => acc + (lowerText.match(new RegExp(kw, 'g')) || []).length, 0);
    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
        try {
          ...
          if (base64Image.length < 5000) continue;
  ...
  async uploadImageToCloudinary(base64DataUrl, pageNum) {
    try {
      ...
      formData.append('imageFile', blob, `pdf_page_${pageNum}`);
