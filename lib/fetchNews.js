...
/* -------------------------------
   NEW: Publication Search Helper
--------------------------------*/
let cachedPublications = [];

export async function getPublications(country = 'us') {
  // Return from cache if available
  if (cachedPublications.length) return cachedPublications;
  ...
  // Combine and extract unique publications
  const allStories = [...uploadedStories, ...externalNews];
  const uniquePublications = Array.from(
    new Map(
      allStories
        .filter(story => story.source_id) // only those with source_id
        .map(story => [
          story.source_id.toLowerCase(),
          {
            source_id: story.source_id,
            name: story.source_id, // can be replaced with display name if available
            image_url: story.image_url || '/default-placeholder.png'
          }
        ])
    ).values()
  );

  cachedPublications = uniquePublications;
