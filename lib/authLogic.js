...
export const handlePublisherRegistration = async (formData, router, setIsLoading) => {
  ...
  try {
    ...
    // Save additional data to Firestore via API with news sources integration
    ...
    const res = await fetch('/api/signup', {
      ...
      body: JSON.stringify({
        ...
        // Additional fields for news sources integration
        isActive: true,
        companyLogo: null, // This can be updated later in profile settings
    ...
    if (result.success) {
      ...
      console.log(' Publisher will now appear in news sources');
...
// Update publisher article stats when they publish content
export const updatePublisherStats = async (publisherId) => {
  try {
    const response = await fetch('/api/update-publisher-stats', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (result.success) {
      ...
      console.error('Failed to update publisher stats:', result.error);
    }
  } catch (error) {
