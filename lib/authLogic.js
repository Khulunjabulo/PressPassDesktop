...
const checkRequiredFieldsComplete = (user) => {
  // Required fields for Legal Requirements
  ...
  console.log('🔍 Checking required fields for user:', user?.email);
  console.log('📊 User data:', {
    businessRegistrationNumber: user?.businessRegistrationNumber,
    publishingLicense: user?.publishingLicense,
  ...
  const legalComplete = legalRequirements.every(field => {
    const value = user?.[field];
    const isComplete = value && value !== '' && value !== null && value !== undefined;
    console.log(`📋 ${field}: ${isComplete ? '✅' : '❌'} (value: ${value})`);
    return isComplete;
  });
  
  const personalComplete = personalInfo.every(field => {
    const value = user?.[field];
    const isComplete = value && value !== '' && value !== null && value !== undefined;
    console.log(`👤 ${field}: ${isComplete ? '✅' : '❌'} (value: ${value})`);
    return isComplete;
  ...
  const allComplete = legalComplete && personalComplete;
  console.log(`🎯 Required fields status - Legal: ${legalComplete}, Personal: ${personalComplete}, All Complete: ${allComplete}`);
...
export const handleSignIn = async (email, password, role, keepSignedIn, router, setError, setLoading) => {
  ...
  try {
    ...
    // MODIFIED: Simplified redirect logic for sign-in
    ...
    if (role === 'reader') {
      ...
      // CHANGED: Publishers always go to overview when signing in
...
const handleGoogleSignInCallback = async (response, role, keepSignedIn, router, setError, setLoading) => {
  ...
  try {
    ...
    // MODIFIED: Simplified redirect logic for Google sign-in
    console.log('🔄 Redirecting user based on role:', role);
...
export const completeGoogleSignUp = async (googleUserData, formData, profilePicPreview, router, setIsLoading) => {
  ...
  try {
    ...
    if (response.data.userData.role === 'publisher') {
  const publisherId = response.data.userData.uid || response.data.userData.id;
  localStorage.setItem('currentPublisherId', publisherId);
  console.log('✅ Publisher ID stored:', publisherId);
}

    // MODIFIED: Publishers go to profile after sign-up, readers go to news-reader
...
// Handle Google Sign-Up Button Click (Legacy - kept for compatibility)
    ...
    try {
      window.google.accounts.id.prompt((notification) => {
        ...
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          console.log('🔄 Google prompt not shown, trying alternative...');
          // If One Tap doesn't work, we could show a message or try another approach
        }
      });
    } catch (error) {
